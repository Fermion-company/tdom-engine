# 第9章 Visual Fidelity Gate — TeX品質を壊さないリアルタイムプレビュー

この章は 2026-07 の第2次改修で導入された**表示品質の安全判定**を説明します。
第8章の safety gate が「JSページ組み立てを信用してよいか（ページ機構）」を
判定するのに対し、この gate は「**ブラウザのグリフ描画がLuaLaTeX出力と
見た目一致するか**」を行・run・フォント単位で判定します。

## 9.1 何が問題だったか

改修前の provisional 層は、数式を含む行も glyph display list（ブラウザSVG
`<text>`）で描いていました。座標はTeX自身の計算なので位置は正確ですが、
**字形のソース**に3つの破れ目がありました。
...
「速いが汚い数式」は絶対条件違反です。canonical 層は最終的に必ず勝ちます
が、**打鍵直後の provisional 表示自体もTeX品質でなければならない**、が
このフェーズの要求でした。
...
2. **stale chunk** — 直前レンダーのピクセル（`st:1` でマーク、~100–200ms
   だけ古い）。「一瞬古いが綺麗」は許容、「速いが汚い」は不許容
...
1. **常駐 RENDER = 編集されたブロック専用**（fork＋再組版＋pdftocairo、
   実測100–200ms級）。ポンプに乗るのは**その編集で dirty になった
   ブロックだけ**。コールドなブロック（boot直後のバックログ等）は
   絶対にポンプへ積まない — luatexja 深系譜では常駐 RENDER の fork 子が
   luahbtex 内部でスピンし、全文書スイープは CPU を飽和させて fork 組版
   自体を破壊する（実測: 編集応答 4s→126s）。RENDER はそのブロック位置の
   checkpoint を必要とするため、**render hold**: dirty になりそうな
   ブロックの off-grid checkpoint は chunk が届くまで退役を保留（上限
   `TDOM_RENDER_HOLD_MAX`=8）。タイムアウト（`TDOM_RENDER_TIMEOUT`、
   既定20s）時はスピンする子を **SIGKILL** して隔離キューへ引き継ぐ。
...
| 要求 | 実装 |
|---|---|
| display math がTeX品質 | math 行は exact chunk（checkpoint子 RENDER） |
...
| 全文 compile を同期で待たない | レンダーポンプは完全非同期・編集はロック外 |
