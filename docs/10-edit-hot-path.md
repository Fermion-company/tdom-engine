...
- resume の walk も予算で cold stop するが、自分の dirty block（`coldDirty`・`coldIds`・galley の無い block）を 1 つ組版するまでは止まらない。walk は chain pass が着いた境界より前から始まることがある（入口境界が保持されていない exact 近傍 block への `firstDisplay` の拡張、別の walk が組版済みの `coldIds` の block）。そこで止まると同じ block がそのまま積み直され、次の resume も同じ場所から始まる。この規則により、resume を重ねるたびに対象の block は少なくとも 1 つずつ減る。
