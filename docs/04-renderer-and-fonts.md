...
- **パッチ適用**：`replace-page` のSVG・chunk・フォント・入力座標・ソース対応を画面外で用意し、
  取引ごとに、すべて揃ってから影響するページ群を同時に差し替える。`pending-exact` やstale chunkがある間は直前の完成した紙面を保持する。ページ数の減少は確定PDFの提示まで保留する。
  取引は `provisionalCommitGroups` が決める。連続するページ（改ページを越えて流れる内容）、同じ block を描くページ（新しい表示リスト同士、または一方の新しい表示リストと、他方の最後に差し替えた表示リスト・段取り中の表示リストのどちらかで同じ block。変わらない float のページを挟んで段落が分かれた・丸ごと移った場合）、1 つの編集のページ（report の `dirtySourceNodes`・`removedSourceNodes` の block を描く・描いていたページ。段落の分割・結合は block id を変える）が 1 つの取引になる。canonical のページ数が分かっていて resident と一致し、段取り中のページにすべて殻があれば、揃った取引から先に差し替える。前の打鍵のページや、編集が触った柱などの離れたページが exact 画素を待っていても、今打っているページを待たせない（同じ取引に入っていれば待つ）。その間、離れたページの番号が一時的に遅れることはある。ページ数が分からない・合わない間、ページの削除を伴う間、直接編集中は、従来どおり全ページを一度に差し替える。
- **SSE**：`update`（他ウィンドウの編集）と `patches`（レンダー子完了
  やバックグラウンド連鎖からの非同期差分）を受けて同じ適用経路へ
- **直接編集**：通常のクリックは表示世代のPDF文字座標と `/dom` のソース範囲を
  対応させて文字・数式を編集する。Cmd/Ctrl付きクリックはソースへ移動する。
