...
4. 本文と inline math の混在行は、本文を現在の TeX run glyph、数式を
...
6. 数式判定をフォント名だけに依存させない。`daemon.lua` が TeX の inline
   ...
   `\mathbf` なども同じ経路で扱う。
7. `/dom` の inline math source region と画面の math run group が一対一に
...
| `engine/checkpoint/display-list.js` | math run と TeX の幅・高さ・深さを SVG data 属性へ渡す |
...
exact chunk 側では `engine/checkpoint/display-list.js` の 0.5bp bleed、block origin
への anchor、zero-area rule の非描画も同じ基準点に含む。これらは上付き文字の
欠け、段落字下げの二重適用、余計な縦線を防ぐ。
...
2026-08-27 のローカル確認では、上記 pure test 28件と bounded hot-path 13件が
